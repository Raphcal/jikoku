...
class ShootingStyle <Definition : ShootingStyleDefinition> {
    ...
    init(definition: Definition, spriteFactory: SpriteFactory) {
...
class StraightShootingStyle : ShootingStyle<StraightShootingStyleDefinition> {
    
    override func shots(from point: Point<GLfloat>) -> [Sprite] {
        var shots = [Sprite]()
        
        var left = point.x - (GLfloat(shotAmount - 1) * definition.space) / 2
